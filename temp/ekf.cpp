...
{
  colvec xk(nOutputs_);//???????????应该是xk(nStates_)
  xk = xk.zeros();
  return xk;
}
...
{
  colvec f0 = f(x, u);//给定x,u后有一个对应的数值
  ...
  for (int j = 0; j < nStates_; j ++) {
    ...
    fn = f(x_eps, u);//第J个变量构造小的增量。求出数值
    
    for (int i = 0; i < nStates_; i ++) {
      F_(i, j) = (fn(i) - f0(i)) / epsilon_;//求出对第I个变量的数值差分    
    }
  }
...
{  
  ...
  CalcF(x_m_, u);//在上次xk_hat基础上求F_
    
  // Prior update:
  x_p_ = f(x_m_, u);//////////////////////////////////predict 求得x_tiuta
  P_p_ = F_ * P_m_ * trans(F_) + Q_;//////////////////predict
  
  CalcH(x_p_);//在x_tiuta基础上更新求得H_
  
  // Measurement update:
  mat K = P_p_ * trans(H_) * inv(H_ * P_p_ * trans(H_) + R_);
  x_m_ = x_p_ + K * (z_ - h(x_p_));//在x_tiuta基础上更新x_hat
  P_m_ = P_p_ - K * H_ * P_p_;
